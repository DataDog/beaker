...
def aesEncrypt(data, key):
    ...
    key_obj = nss.nss.import_sym_key(
        slot,
        _mech,
        nss.nss.PK11_OriginGenerated,
        nss.nss.CKA_ENCRYPT,
        nss.nss.SecItem(key),
    )

    param = nss.nss.param_from_iv(_mech, nss.nss.SecItem(_iv))
    ctx = nss.nss.create_context_by_sym_key(_mech, nss.nss.CKA_ENCRYPT, key_obj, param)
...
def aesDecrypt(data, key):
    ...
    param = nss.nss.param_from_iv(_mech, nss.nss.SecItem(_iv))
    ctx = nss.nss.create_context_by_sym_key(_mech, nss.nss.CKA_DECRYPT, key_obj, param)
    l1 = ctx.cipher_op(data)
    # Yes, DIGEST.  This needs fixing in NSS, but apparently nobody (including
    # me :( ) cares enough.
    l2 = ctx.digest_final()
