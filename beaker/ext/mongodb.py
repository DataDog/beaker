    ...
    *default database*, so make sure your connection string or
    ...
    def set_value(self, key, value, expiretime=None):
        ...
        self.db.backer_cache.update_one(
            ...
            {"$set": {"value": bson.Binary(value), "expiration": expiration}},
            upsert=True,
        )
    ...
    def keys(self):
        return [
            e["key"].split(":", 1)[-1]
            for e in self.db.backer_cache.find_all(
    ...
    *default database*, so make sure your connection string or
    ...
    def _clear_expired_locks(self):
        ...
        self.db.beaker_locks.delete_many(
            {"_id": self.identifier, "timestamp": {"$lte": expired}}
        )
        return now
    ...
    def do_release_read_lock(self):
        ...
        self.db.beaker_locks.update_one(
            {"_id": self.identifier, "readers": owner_id},
    ...
    def do_acquire_read_lock(self, wait):
        ...
        while True:
            try:
                self.db.beaker_locks.update_one(
                    {"_id": self.identifier, "owner": None},
                    {"$set": {"timestamp": now}, "$push": {"readers": owner_id}},
                    upsert=True,
                )
                return True
    ...
    def do_release_write_lock(self):
        self.db.beaker_locks.delete_one(
            {"_id": self.identifier, "owner": self._get_owner_id()}
        )
    ...
    def do_acquire_write_lock(self, wait):
        ...
        while True:
            try:
                self.db.beaker_locks.update_one(
                    {"_id": self.identifier, "owner": None, "readers": []},
                    {"$set": {"owner": owner_id, "timestamp": now}},
                    upsert=True,
                )
                return True
            except pymongo.errors.DuplicateKeyError:
