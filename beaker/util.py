...
def coerce_session_params(params):
    rules = [
        (
            "data_dir",
            (str, NoneType),
            "data_dir must be a string referring to a directory.",
        ),
        (
            "lock_dir",
            (str, NoneType),
            "lock_dir must be a string referring to a directory.",
        ),
        ("type", (str, NoneType), "Session type must be a string."),
        (
            "cookie_expires",
            (bool, datetime, timedelta, int),
        ...
        (
            "encrypt_nonce_bits",
            (int, NoneType),
            "Session encrypt_nonce_bits must be a number",
        ),
    ...
    if (
        cookie_expires
        and isinstance(cookie_expires, int)
        and not isinstance(cookie_expires, bool)
    ):
...
def coerce_memcached_behaviors(behaviors):
    rules = [
        ...
        (
            "ketama_weighted",
            (bool, int),
            "ketama_weighted must be a boolean or an integer",
        ),
        (
            "_io_key_prefetch",
            (int, bool),
            "_io_key_prefetch must be a boolean or an integer",
        ),
        (
        ...
        (
            "buffer_requests",
            (bool, int),
            "buffer_requests must be a boolean or an integer",
        ),
...
def parse_cache_config_options(config, include_defaults=True):
    ...
    if include_defaults:
        options = dict(type="memory", data_dir=None, expire=None, log_file=None)
    else:
        options = {}
