...
def simple_app(environ, start_response):
    ...
    msg = "The current value is: %s, session id is %s" % (
        session.get("value", 0),
        session.id,
    )
...
def test_different_domain():
    ...
    app = TestApp(SessionMiddleware(simple_app, **options))
    res = app.get("/", extra_environ=dict(domain=".hoop.com", HTTP_HOST="www.hoop.com"))
    res = app.get(
        "/", extra_environ=dict(domain=".hoop.co.uk", HTTP_HOST="www.hoop.com")
    )
    assert "Domain=.hoop.co.uk" in res.headers["Set-Cookie"]
    assert "current value is: 2" in res

    res = app.get(
        "/", extra_environ=dict(domain=".hoop.co.uk", HTTP_HOST="www.test.com")
    )
    assert "current value is: 1" in res
