...
if not crypto.get_crypto_module("default").has_aes:
    raise SkipTest("No AES library is installed, can't test cookie-only " "Sessions")
...
def test_increment_with_encryption():
    options = {
        "session.encrypt_key": "666a19cf7f61c64c",
        "session.validate_key": "hoobermas",
        "session.type": "cookie",
    }
    app = TestApp(SessionMiddleware(simple_app, **options))
...
def test_changing_encrypt_key_with_timeout():
    ...
    options = {
        "session.encrypt_key": COMMON_ENCRYPT_KEY,
        "session.timeout": 300,
        "session.validate_key": "hoobermas",
        "session.type": "cookie",
    }
    ...
    options = {
        "session.encrypt_key": DIFFERENT_ENCRYPT_KEY,
        "session.timeout": 300,
        "session.validate_key": "hoobermas",
        "session.type": "cookie",
    }
