...
def test_legacy_cache():
    ...
    assert (
        cache.get_value(
            "x", expiretime=1, createfunc=lambda: "6", type="file", data_dir="./cache"
        )
        == "6"
    )
    ...
    assert (
        cache.get_value(
            "x", expiretime=1, createfunc=lambda: "8", type="file", data_dir="./cache"
        )
        == "6"
    )
    time.sleep(1)
    ...
    assert (
        cache.get_value(
            "x", expiretime=1, createfunc=lambda: "10", type="file", data_dir="./cache"
        )
        == "10"
    )
    ...
    assert (
        cache.get_value(
            "x", expiretime=1, createfunc=lambda: "12", type="file", data_dir="./cache"
        )
        == "10"
    )
...
def test_upgrade():
    ...
    for test in _test_upgrade_has_key, _test_upgrade_in, _test_upgrade_setitem:
        for mod, tar in (("dbm", dbm_cache_tar), ("dumbdbm", dumbdbm_cache_tar)):
            try:
                __import__(mod)
            except ImportError:
                continue
