...
def check_save_load_encryption(session_getter):
    ...
    if not get_crypto_module("default").has_aes:
        raise SkipTest()
    session = session_getter(encrypt_key="666a19cf7f61c64c", validate_key="hoobermas")
    session[u_("Suomi")] = u_("Kimi Räikkönen")
    session[u_("Great Britain")] = u_("Jenson Button")
    session[u_("Deutchland")] = u_("Sebastian Vettel")
    session.save()
    ...
    session = session_getter(
        id=session.id, encrypt_key="666a19cf7f61c64c", validate_key="hoobermas"
    )
    assert u_("Suomi") in session
    assert u_("Great Britain") in session
    assert u_("Deutchland") in session
...
def test_timeout_requires_accessed_time():
    ...
    assert_raises(BeakerException, get_session, timeout=2, save_accessed_time=False)
...
def test_cookies_enabled():
    ...
    cookie = session.request[
        "cookie_out"
    ].lower()  # Python3.4.3 outputs "Secure", while previous output "secure"
...
def test_file_based_replace_optimization():
    ...
    session = get_session(
        use_cookies=False, type="file", data_dir="./cache", id=session.id
    )
...
def test_invalidate_invalid_signed_cookie():
    ...
    COOKIE_REQUEST["cookie_out"] = (
        COOKIE_REQUEST["cookie_out"][:20] + "aaaaa" + COOKIE_REQUEST["cookie_out"][25:]
    )

    assert_raises(
        BeakerException, get_cookie_session, id=session.id, invalidate_corrupt=False
    )
...
class TestSaveAccessedTime(unittest.TestCase):
    ...
    def test_saves_if_session_written_and_accessed_time_false(self):
        ...
        session.save()
        session = get_session(
            data_dir="./cache", save_accessed_time=False, id=session.id
        )
        ...
        assert session.last_accessed > last_accessed, "%r is not greater than %r" % (
    ...
    def test_saves_if_session_not_written_and_accessed_time_true(self):
        ...
        session.save()
        session = get_session(
            data_dir="./cache", save_accessed_time=True, id=session.id
        )
        last_accessed = session.last_accessed
